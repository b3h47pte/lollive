...
std::string LeagueImageAnalyzer::AnalyzePlayerChampion(uint idx, ELeagueTeams team, bool* isDead, bool* isLowOnHealth, int* championLevel) {
  ...
  bool tmpIsDead = false;
  bool tmpIsLowOnHealth = false;
  std::string championMatch = FindMatchingChampion_Histogram(filterImage, allHints, tmpIsLowOnHealth, tmpIsDead, false);
  if (isDead) {
    *isDead = tmpIsDead;
  }
